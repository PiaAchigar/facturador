...
/**
 * Perdonar un vencimiento (botón "Ignorar" del aviso, 2026-09-10).
 *
 * Laura mira el aviso, decide que esa plata se la deja a la clienta igual, y el
 * aviso deja de nombrarla. La plata NO se toca.
 */
describe("lotes perdonados", () => {
  /** Una acreditación que pasó de fecha y que Laura decidió perdonar. */
  function perdonada(fecha: string, monto: number, vence: string, perdonadaEl: string) {
    return { ...acredita(fecha, monto, vence), expiryIgnoredAt: d(perdonadaEl) };
  }

  it("un lote perdonado sale del aviso pero sigue siendo plata de la clienta", () => {
    const r = lotesDeSaldo(
      [perdonada("2026-04-09T10:00:00Z", 80000, "2026-07-09T10:00:00Z", "2026-09-10T10:00:00Z")],
      AHORA,
    );
    expect(r.vencido).toBe(0);
    expect(r.ignorado).toBe(80000);
  });

  it("perdonar NO lo vuelve vigente: son dos cosas distintas", () => {
    const r = lotesDeSaldo(
      [perdonada("2026-04-09T10:00:00Z", 80000, "2026-07-09T10:00:00Z", "2026-09-10T10:00:00Z")],
      AHORA,
    );
    // Si sumara a `vigente` la ficha diría que está dentro de sus 3 meses, y no
    // lo está: está perdonada, que es una decisión de Laura, no un plazo.
    expect(r.vigente).toBe(0);
  });

  /**
   * El motivo por el que la marca va en el movimiento y no en la clienta. Con
   * una marca por clienta este test fallaría: el lote de agosto también
   * quedaría callado, y sería plata que nadie miró nunca.
   */
  it("perdonar un lote no silencia otro que venza después", () => {
    const r = lotesDeSaldo(
      [
        perdonada("2026-01-01T10:00:00Z", 80000, "2026-04-01T10:00:00Z", "2026-05-01T10:00:00Z"),
        acredita("2026-05-01T10:00:00Z", 30000, "2026-08-01T10:00:00Z"),
      ],
      AHORA,
    );
    expect(r.ignorado).toBe(80000);
  ...
  it("la plata perdonada se sigue gastando primero, por vieja", () => {
    const r = lotesDeSaldo(
      [
        perdonada("2026-01-01T10:00:00Z", 80000, "2026-04-01T10:00:00Z", "2026-05-01T10:00:00Z"),
        acredita("2026-08-01T10:00:00Z", 50000, "2026-11-01T10:00:00Z"),
        gasta("2026-09-01T10:00:00Z", 80000),
      ],
      AHORA,
    );
    // El consumo se comió entero el lote perdonado; queda sólo el vigente.
    expect(r.ignorado).toBe(0);
    expect(r.vigente).toBe(50000);
  });
});
